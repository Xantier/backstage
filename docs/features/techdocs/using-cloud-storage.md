...
**3. Create minimal AWS IAM policies to manage tech docs**

To *write* tech docs into the S3 bucket the IAM policy needs to have at a minimum permissions to:
* `s3:ListBucket` to retrieve bucket metadata
* `s3:PutObject` to upload files to the bucket

To *read* tech docs from the S3 bucket the IAM policy needs to have at a minimum permissions to:
* `s3:ListBucket` - To retrieve bucket metadata
